...
Hedging (e.g. using stock index futures to hedge beta risk in portfolios) 
often ends up with using "ARMAX(0,0,1)/GARCH(1,1)" model to compute the hedge ratio.
This is because a simple linear regression is not able to capture 
stochasticity in volatilities. 
 The JAVA routine implements the MLE of the model parameters. 
 
 The model is described as follows:
x, y are two time series:         y(t) = chi + gamma * x(t) + e(t)
e is residual series:             e(t) ~ Normal(0, h(t))
variance for residual series:     h(t) = omega + alpha * e(t-1)^2 + beta * h(t-1)

Parameter gamma is the hedge ratio. The error term does not just follow a 
Normal Distribution. In the routine, it also assumes the Student-t 
and the exponential power distribution, which are more realistic in financial markets. 
The routine uses BHHH method, a variant of Gauss-Newton method, to perform the 
nonlinear optimization. As a comparison, it also utilizes 
the Nelder-Mead Simplex method. Small difference between the calibrated model 
output and the hypothetical input indicates the correctness of the implementation.
